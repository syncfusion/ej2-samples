...
(window as any).default = (): void => {
    ...
    function onScroll(): void {
        let sliderCollection: HTMLCollection = document.getElementsByClassName('ticks_slider');
        let i: number; let len: number = sliderCollection.length;
        for (i = 0; i < len; i++) {
          (sliderCollection[i] as any).ej2_instances[0].refreshTooltip();
        }
    }
};